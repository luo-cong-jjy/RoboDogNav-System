...
class MissionExecutor(Node):
    ...
    def _run_elevator(self, goal_handle, step: MissionStep):
        ...
        floor = self._floor_state
        if floor is None or not floor.ready:
            return 'FAILED', 'active map is not ready for elevator step'
        ...
        if not recovering_target:
            if floor.floor_id != step.floor_id:
                return 'FAILED', (
                    f'elevator step requires {step.floor_id} or committed '
                    f'{step.target_floor}, active={floor.floor_id}'
                )
            for pose, segment in (
                (step.pose, 'elevator_lobby'),
                (plan.source_trigger, 'elevator_cabin'),
            ):
                outcome, message = self._navigate(
        ...
        request.elevator_id = step.elevator_id
    ...
    def _execute(self, goal_handle):
        ...
        try:
            ...
            while self._step_index < len(self._steps):
                ...
                elif step.step_type == 'elevator_transfer':
                    outcome, message = self._run_elevator(
                        goal_handle, step
                    )
