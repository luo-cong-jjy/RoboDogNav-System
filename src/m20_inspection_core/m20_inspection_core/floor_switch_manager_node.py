...
from m20_warehouse_interfaces.action import SwitchFloor
...
class FloorSwitchManager(Node):
    ...
    def __init__(self) -> None:
        ...
        self._transition_delay = float(
            self._config['simulation']['elevator_transition_delay_sec']
        )
        self._teleport_on_switch = bool(
            self._config['simulation']['teleport_on_floor_switch']
        )
        ...
        self._pose_client = (
            self.create_client(
                SetSimulationPose,
                str(self.get_parameter('set_pose_service').value),
                callback_group=self._callback_group,
            )
            if self._teleport_on_switch
            else None
        )
        ...
        transfer_mode = (
            'teleport'
            if self._teleport_on_switch
            else 'in-place shared gateway'
        )
        self.get_logger().info(
            f'floor switch action manager ready; transfer_mode={transfer_mode}'
        )
    ...
    def _establish_target_pose(self, plan, generation: int):
        """Teleport only for legacy profiles; otherwise verify pose continuity."""
        if not self._teleport_on_switch:
            odom = self._odometry
            if odom is None:
                return False, 'odometry unavailable after map switch'
            current_pose = (
            ...
            distance, yaw_error = pose_error(
                current_pose, plan.target_release
            )
            if (
                ...
                return (
                    False,
                    f'robot moved away from shared gateway by '
                    f'{distance:.3f}m/{yaw_error:.3f}rad',
                )
            return True, 'world pose preserved at shared gateway'
        ...
        if pose_response is None or not pose_response.success:
            return False, 'simulation pose transfer failed'
        return True, 'simulation pose transfer complete'
    ...
    def _execute(self, goal_handle):
        ...
        try:
            ...
            self._feedback(
                goal_handle,
                'ELEVATOR_TRANSIT',
                'simulating elevator travel while motion remains held',
            )
            self._delay_with_cancel(goal_handle, self._transition_delay)
            ...
            self._feedback(
                ...
                (
                    'RELOCATING'
                    if self._teleport_on_switch
                    else 'VERIFYING_SHARED_GATEWAY'
                ),
                (
                    'moving RViz backend to the target elevator lobby'
                    if self._teleport_on_switch
                    else 'verifying unchanged robot pose at the shared origin'
                ),
                generation,
            )
            pose_ready, pose_message = self._establish_target_pose(
            ...
            return self._result(
                ...
                (
                    ...
                    f'without pose transfer'
                    if not self._teleport_on_switch
                    else f'switched {plan.source_floor}->{plan.target_floor}'
                ),
            )
        ...
        except Exception as error:  # Defensive boundary: retain hold on faults.
            if transaction_started:
                self._publish_hold(True)
