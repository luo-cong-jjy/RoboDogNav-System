...
public:
  CmdVelSafetyMuxNode() : Node("cmd_vel_safety_mux") {
    upstream_cmd_topic_ = declare_parameter<std::string>(
      "upstream_cmd_topic", "/m20_nav2/cmd_vel");
    manual_cmd_topic_ = declare_parameter<std::string>(
      "manual_cmd_topic", "/m20_control/cmd_vel_manual");
    e_stop_topic_ = declare_parameter<std::string>(
      "e_stop_topic", "/m20_control/e_stop");
    safe_cmd_topic_ = declare_parameter<std::string>(
      "safe_cmd_topic", "/m20_control/cmd_vel_safe");
    ...
    upstream_subscription_ = create_subscription<geometry_msgs::msg::Twist>(
      upstream_cmd_topic_, 10,
      [this](const geometry_msgs::msg::Twist::SharedPtr msg) {
        last_upstream_cmd_ = *msg;
        last_upstream_time_ = now();
        has_upstream_cmd_ = true;
      });
    ...
    RCLCPP_INFO(
      get_logger(),
      "速度安全仲裁已启动：upstream=%s manual=%s e_stop=%s -> safe=%s",
      upstream_cmd_topic_.c_str(),
...
private:
  ...
  geometry_msgs::msg::Twist selectCommand(
    ...
    const bool upstream_fresh = isFresh(has_upstream_cmd_, last_upstream_time_, current_time);
    ...
    if (manual_priority_) {
      ...
      if (upstream_fresh) {
        source = CommandSource::kUpstream;
        return last_upstream_cmd_;
      }
    } else {
      if (upstream_fresh) {
        source = CommandSource::kUpstream;
  ...
  std::string upstream_cmd_topic_;
