...
public:
    CircleDemo() : Node("m20_circle_demo") {
        ...
        yaw_rate_ = declare_parameter<double>("yaw_rate", 0.30);
        ...
        timer_ = create_wall_timer(std::chrono::duration_cast<std::chrono::milliseconds>(period),
                                   std::bind(&CircleDemo::OnTimer, this));

        RCLCPP_INFO(get_logger(), "Circle demo: topic=%s speed=%.3f yaw_rate=%.3f radius~=%.3fm duration=%.2fs",
                    topic_.c_str(), speed_, yaw_rate_,
                    yaw_rate_ == 0.0 ? 0.0 : speed_ / yaw_rate_, duration_sec_);
    }
...
private:
    void OnTimer() {
        ...
        if (stop_count_ > 20) {
            RCLCPP_INFO(get_logger(), "Circle demo finished.");
            rclcpp::shutdown();
        }
    }
...
int main(int argc, char** argv) {
    ...
    rclcpp::spin(std::make_shared<CircleDemo>());
    return 0;
}
