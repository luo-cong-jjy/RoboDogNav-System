...
"""Generate deterministic PCD and occupancy assets for each logical floor."""
...
from .configuration import display_to_source_xy
...
class FloorAssets:
    ...
    floor_id: str
    pcd_path: Path
    occupancy_yaml_path: Path
    occupancy_image_path: Path
    metadata_path: Path
    point_count: int
    obstacle_count: int
    pcd_sha256: str
...
def _mission_corridor_segments(
    ...
    for step in config['mission']['sequence']:
        ...
        if step_type in {'inspection', 'transit'}:
            ...
            elevator = config['elevators'][str(step['elevator'])]
            source_floor = str(step['from'])
            target_floor = str(step['to'])
            _append(source_floor, elevator['source_trigger_pose'])
            current_floor = target_floor
            current_pose = elevator['target_release_pose']
...
def _write_pcd(path: Path, points: np.ndarray, intensity: float) -> None:
...
def _write_pgm(path: Path, occupancy: np.ndarray) -> None:
    image = np.where(occupancy == 100, 0, 254).astype(np.uint8)
    image = np.flipud(image)
    height, width = image.shape
    header = f'P5\n# M20 deterministic warehouse occupancy\n{width} {height}\n255\n'
    _atomic_bytes(path, header.encode('ascii') + image.tobytes())
...
def _floor_asset_paths(
    ...
    yaml_path = output_root / floor['occupancy_file']
    image_path = yaml_path.with_suffix('.pgm')
    metadata_path = output_root / floor['metadata_file']
    return pcd_path, yaml_path, image_path, metadata_path
...
def generate_floor_assets(
    ...
    _atomic_text(
        yaml_path,
        yaml.safe_dump(occupancy_description, sort_keys=False),
    )
    pcd_hash = sha256_file(pcd_path)
    pgm_hash = sha256_file(image_path)
    ...
    metadata: Dict[str, Any] = {
        ...
        'occupied_cell_count': int(np.count_nonzero(occupancy == 100)),
        'files': {
            'pcd': pcd_path.name,
            'pcd_sha256': pcd_hash,
            'occupancy_yaml': yaml_path.name,
            'occupancy_image': image_path.name,
            'occupancy_image_sha256': pgm_hash,
        },
    }
    _atomic_text(
        metadata_path,
    ...
    return FloorAssets(
        floor_id=floor_id,
        pcd_path=pcd_path,
        occupancy_yaml_path=yaml_path,
        occupancy_image_path=image_path,
...
def read_pgm(path: Path) -> np.ndarray:
    """Read the deterministic binary PGM occupancy image."""
    with Path(path).open('rb') as stream:
        ...
        if int(stream.readline()) != 255:
            raise ValueError(f'{path} does not use 8-bit pixels')
        data = stream.read()
    image = np.frombuffer(data, dtype=np.uint8)
    if image.size != width * height:
        raise ValueError(f'{path} has an invalid pixel count')
...
def validate_generated_assets(
    ...
    """Check generated files, hashes, bounds, counts, and replica contracts."""
    root = Path(package_root).resolve()
    results: List[FloorAssets] = []
    pcd_hashes: Dict[str, str] = {}
    pcd_values: Dict[str, np.ndarray] = {}
    pgm_values: Dict[str, np.ndarray] = {}
    ...
    for floor_id, floor in config['floors'].items():
        ...
        for path in (pcd_path, yaml_path, image_path, metadata_path):
            if not path.is_file():
        ...
        with yaml_path.open('r', encoding='utf-8') as stream:
            occupancy_description = yaml.safe_load(stream)
        ...
        if metadata['files']['occupancy_image_sha256'] != pgm_hash:
            raise ValueError(f'{metadata_path} PGM hash does not match')
        if occupancy_description['image'] != image_path.name:
            raise ValueError(f'{yaml_path} image reference does not match')
        if image.shape != (
            int(round(2.0 * expected_half_y / occupancy_description['resolution'])),
            int(round(2.0 * expected_half_x / occupancy_description['resolution'])),
        ):
            raise ValueError(f'{image_path} dimensions do not match configuration')
        ...
        if replica_of is None:
            ...
            if not np.array_equal(
                image[1:-1, 1:-1],
                pgm_values[replica_of][1:-1, 1:-1],
            ):
                raise ValueError(
                    f'{floor_id} occupancy interior is not a copy of '
                    f'{replica_of}'
                )
