...
class Phase5Regression(Node):
    ...
    def _source_trigger(self, source: str, target: str):
        elevator = self.config['elevators']['E1']
        if (
            source == elevator['source_floor']
            and target == elevator['target_floor']
        ):
            values = elevator['source_trigger_pose']
        else:
            values = self.config['floors'][source]['elevator_cabin_pose']
    ...
    def _target_release(self, source: str, target: str):
        ...
        if (
            ...
            values = elevator['target_release_pose']
        else:
            values = self.config['floors'][target]['elevator_lobby_pose']
        return tuple(float(value) for value in values)
