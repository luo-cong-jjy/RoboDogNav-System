...
/move_base_simple/goal
  geometry_msgs/msg/PoseStamped
  原版 SCAN 的 RViz/人工目标入口。

/m20/navigation/goal_pose
  geometry_msgs/msg/PoseStamped
  可选栅格路线适配器的外部目标入口；默认 scan_native profile 不使用。
...
/m20/navigation/global_route_state
  std_msgs/msg/String
  READY、TRACKING_ROUTE、GOAL_REACHED、NO_ROUTE、REJECTED_NOT_READY、
  REJECTED_FLOOR_SWITCH 或 FLOOR_SWITCH_HOLD。
...
/m20/navigation/route_reset
  ...
  清空栅格 A* 路线、短子目标、路线事件和进度；不替代 SCAN GridMap reset。
...
默认 `scan_native` 模式把人工目标和 typed 自动任务目标都送入原版
`/move_base_simple/goal -> SCAN rebound/B-spline -> closed-loop controller` 链。
active occupancy 上的 A* 和约 0.9 m 短子目标仅由 `use_grid_route:=true` 显式启用，
不属于日常启动路径。`/m20/navigation/initial_path` 保留为实验接口。
...
/m20/control/collision_guard_diagnostic
  std_msgs/msg/String
  首个碰撞样本的当前/预测类型、时间、前/后圆、坐标、栅格、原因和候选速度。
...
/m20/sim/body_pose
  ...
  RViz 运动学后端输出；实机 profile 通过同一语义接口替换。
...
急停、切图保持和任务保持必须立即输出零速度，不经过普通减速度限制。自动链路固定为
