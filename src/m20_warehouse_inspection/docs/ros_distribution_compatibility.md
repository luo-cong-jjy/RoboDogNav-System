...
当前七包项目集合在 Ubuntu 22.04 / Humble 已完成独立构建、自动测试和运行时回归。
Foxy 只完成源码与接口静态审计，不能标记为“已构建”或“已运行”：

```text
Humble clean build/runtime        VERIFIED
Python 3.8 grammar                STATIC PASS（51 files）
...
优先策略是让 SCAN、地图、任务和安全层运行在随车 Humble 计算机，Foxy 只保留厂商
SDK/控制侧 adapter：

```text
Humble navigation computer
  RunMission / NavigateFloor / SwitchFloor
  map + sensing + SCAN + safety
  /m20/control/cmd_vel_safe
             |
             | 明确版本的窄接口
             v
Foxy/SDK control computer
  M20 motion adapter
  watchdog + physical e-stop + remote takeover
```

这样能把高风险 backport 限制在小型 adapter，而不是把完整 SCAN 和任务栈迁回已 EOL
的发行版。若现场要求单机 Foxy，再执行第 5 节的原生验证矩阵。
...
3. 构建 13 包 closure，不复用 Humble 产物。
