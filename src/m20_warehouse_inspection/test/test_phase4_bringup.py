...
def _source(package: str, relative: str) -> str:
    return (SOURCE_ROOT / package / relative).read_text(encoding='utf-8')
...
def test_mission_uses_typed_child_actions_and_lobby_cabin_approach() -> None:
    source = _source(
        'm20_inspection_core',
    ...
    assert "(step.pose, 'elevator_lobby')" in source
    assert "(plan.source_trigger, 'elevator_cabin')" in source
...
def test_connected_profile_never_creates_or_calls_pose_transfer_client() -> None:
    switch = _source(
        'm20_inspection_core',
        'm20_inspection_core/floor_switch_manager_node.py',
    )
    assert 'if self._teleport_on_switch' in switch
    assert 'if not self._teleport_on_switch:' in switch
    assert "else 'VERIFYING_SHARED_GATEWAY'" in switch
    assert "'verifying unchanged robot pose at the shared origin'" in switch
    assert "'without pose transfer'" in switch
