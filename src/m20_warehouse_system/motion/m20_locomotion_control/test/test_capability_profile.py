...
def test_factory_profile_is_separate_from_mujoco_policy_measurements() -> None:
    ...
    assert profile.supports_zero_radius_yaw
    assert not profile.supports_reverse_tracking
    assert profile.minimum_centerline_turn_radius == 0.0
    assert profile.measurement_source == 'factory_motion_status'
    assert profile.positive_yaw_lateral_drift == 0.0
