...
- Added a geometry-only M20 clearance overlay without changing native SCAN
  search, B-spline, controller, or visualization logic. Complete RViz,
  MuJoCo, and hardware compositions now default to
  `clearance_m20.yaml`: the oriented double-circle changes from upstream
  `radius=0.25, offset=0.18, dist0=0.20 m` to
  `radius=0.28, offset=0.18, dist0=0.17 m`. Its axial hard envelope grows
  from about `0.86 x 0.50 m` to `0.92 x 0.56 m` around the confirmed
  `0.82 x 0.51 m` M20 body, while the nominal one-sided band remains
  `0.45 m`. The exact upstream geometry remains selectable with
  `clearance_vendor.yaml`. Added axial-coverage calculations and regression
  checks; the double circle remains a traversability approximation rather
  than a strict rectangular-corner enclosure.
- Made navigation-gateway guard readiness explicit per execution profile:
  `scan_native` no longer waits for a guard node that is intentionally not
  launched, while `m20_safe` and hardware retain fail-closed `CLEAR`
  gating. The controlled narrow-passage launch now also defaults to the
  same `m20` clearance profile as the production compositions.

## Unreleased - 2026-08-11

### Current baseline

- Restored upstream SCAN execution semantics as the default
  `execution_profile:=scan_native`. The default graph now sends the original
  holonomic controller Twist through only the indispensable floor/e-stop gate;
  it does not start the M20 rolling adapter, footprint collision guard,
  recovery state machine, or external trajectory-clock hold. Removed the
  M20-only short-route B-spline time scaling, post-hold derivative reset, and
  near-goal planner exit. `planner_manager.cpp` is logic-identical to
  `src/third_party/SCAN-Planner`; floor-map reset remains the explicit
  integration seam. The former chain is retained behind
  `execution_profile:=m20_safe`, and real-hardware launch remains pinned to
  that fail-closed profile pending physical acceptance. Headless RViz reached
  a 2 m goal with 0.026 m error; the complete official-SDK + MuJoCo one-click
  chain reached a 1.5 m goal with 0.150 m error and no backend fault. The five
  affected packages passed 211/211 source tests. See
  [the restoration record](docs/devlog/2026-08-11_scan_native_execution_restore.md)
  and [the regression report](docs/test_reports/2026-08-11_scan_native_execution_restore.md).

- Replaced the in-physics-process MuJoCo tracking viewer with an isolated,
  read-only, frame-capped MuJoCo/GLFW display. The default view is a direct
  top-down 4 m follow camera; the viewer runs at nice 10 and uses a low-cost
  display replica, while physics remains headless. On the WSLg GTX 1660 host,
  viewer CPU fell from about 90.1% to 39.3%. A complete SCAN + official policy
  + MuJoCo run reached a 2 m goal with backend fault empty and collision guard
  CLEAR. See
  [the viewer optimization record](docs/devlog/2026-08-11_mujoco_lightweight_follow_viewer.md).

- Simplified the simulation and deployment map contract to PCD + JSON only.
  Removed all generated PGM/YAML assets, the project-added static-grid A*
  executable, sequential SCAN subgoals, route-reset/segment-hold runtime
  branches, and their RViz displays. Manual and typed goals now share the
  native SCAN goal, GridMap, rebound, B-spline, controller, and visualization
  chain. The independent M20 collision guard now rasterizes SCAN's live
  `/grid_map/occupancy` PointCloud2 in memory and fails closed on stale cloud;
  bounded recovery can reset and retry native SCAN but cannot switch planning
  algorithms. The complete source test set passed 157/157 and the headless
  RViz-kinematic smoke reached a 2 m goal with 7.3 mm final error, guard CLEAR,
  and no A-star/collision error. This entry supersedes the static-grid route
  bullets retained below as development history. See
  [the migration record](docs/devlog/2026-08-11_pcd_only_native_scan_migration.md)
  and [the regression report](docs/test_reports/2026-08-11_pcd_only_native_scan_regression.md).

- Added the Ubuntu 20.04 / ROS 2 Foxy single-host hardware profile around the
  validated Elevator-LIO topics. The real launch can start LIO, consumes its
  world-frame deskewed cloud, and overlays factory MotionStatus velocity on
  `/LIO/odom_vehicle` so route-stop and floor-switch checks do not mistake
  LIO's empty body twist for a confirmed stop. Added Python 3.8 annotation and
  rclpy shutdown compatibility fixes, a Foxy source preflight, and a guarded
  transport selector. The complete development guide now supplies the exact
  high-level DrDDS schema; the top-level `drdds` is the canonical union package
  and direct ROS is implemented alongside the default basic_server backend.
  Target Fast DDS/QoS validation still gates real motion. See
  [the Foxy/Elevator-LIO integration record](docs/real_robot_migration/03_foxy_x86_elevator_lio_integration.md)
  and [the dual-transport validation report](docs/test_reports/2026-08-11_factory_dual_transport_and_motion_feedback.md).
