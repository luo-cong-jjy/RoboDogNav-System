...
# ============================================================================
# 文件：route_challenge_mission_rviz.launch.py
# 功能：启动「路线障碍挑战（route-obstacle challenge）」专项配置的
#       RViz 仿真巡检任务。本 launch 是轻量入口：只负责选定挑战场景的
#       系统配置文件（route_challenge_system.yaml），并把 use_rviz /
#       use_planner / run_acceptance / acceptance_mode 等参数透传给
#       完整的巡检任务 launch（inspection_mission_rviz.launch.py），
#       从而复用其冻结（frozen）的 RViz 显示图与完整任务图。
# ============================================================================

"""Launch the isolated route-obstacle challenge profile."""
...
def generate_launch_description() -> LaunchDescription:
    """Select the challenge profile while reusing the frozen RViz graph."""
    # —— 路径准备：解析本包 share 目录并拼接挑战配置文件路径 ——
    integration = Path(  # 本包安装后的 share 目录（存放 config/launch 等）
        get_package_share_directory('m20_warehouse_inspection')
    )
    system_config = integration / 'config' / 'route_challenge_system.yaml'  # 路线障碍挑战场景的系统配置文件
    # —— launch 参数（可被命令行覆盖）——
