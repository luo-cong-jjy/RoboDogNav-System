...
def test_shipped_configuration_is_valid_and_flat() -> None:
    ...
    assert [
        (route['from'], route['to'])
        for route in elevator['transitions']
    ] == [('F1', 'F2'), ('F2', 'F1')]
    assert all(
        route['source_trigger_pose'] == [0.0, 0.0, 0.0]
        and route['target_release_pose'] == [0.0, 0.0, 0.0]
        for route in elevator['transitions']
    )
    assert config['floor_switch'] == {
...
def test_floor_switch_rejects_unknown_adapter_and_duplicate_route() -> None:
    config = deepcopy(load_system_config(CONFIG_PATH))
    config['floor_switch']['transfer_adapter'] = 'magic'
    with pytest.raises(ConfigurationError, match='transfer_adapter'):
        validate_system_config(config)

    config = deepcopy(load_system_config(CONFIG_PATH))
    routes = config['elevators']['E1']['transitions']
    routes.append(deepcopy(routes[0]))
    with pytest.raises(ConfigurationError, match='duplicates route'):
        validate_system_config(config)


def test_generic_mission_transfer_requires_a_supported_route() -> None:
    config = deepcopy(load_system_config(CONFIG_PATH))
    step = config['mission']['sequence'][2]
    validate_system_config(config)

    step['to'] = 'F1'
    with pytest.raises(
        ConfigurationError,
        match='unsupported connector route',
    ):
        validate_system_config(config)
