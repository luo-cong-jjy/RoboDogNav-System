...
def main_validate(arguments: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        '--require-assets',
        action='store_true',
        help='also validate all configured PCD and JSON metadata files',
    )
    ...
    try:
        ...
        if options.require_assets:
            assets = validate_map_assets(config, options.package_root)
            for asset in assets:
                print(
...
def main_import_site_pcd(
    arguments: Optional[Sequence[str]] = None,
) -> int:
    """Import an Elevator-LIO ASCII PCD into the audited site-map contract."""
    parser = argparse.ArgumentParser(
        description='Normalize and audit one surveyed M20 site PCD'
    )
    parser.add_argument('--input', type=Path, required=True)
    parser.add_argument('--output-pcd', type=Path, required=True)
    parser.add_argument('--output-metadata', type=Path, required=True)
    parser.add_argument('--floor', required=True)
    parser.add_argument('--asset-frame', default='world')
    parser.add_argument('--source-map-builder', default='Elevator-LIO')
    ...
    try:
        asset = import_surveyed_ascii_pcd(
            ...
            options.floor,
            asset_frame=options.asset_frame,
            source_map_builder=options.source_map_builder,
            force=options.force,
        )
    except (FileExistsError, FileNotFoundError, KeyError, ValueError) as error:
        parser.error(str(error))
    print(
        f'imported surveyed {asset.floor_id}: {asset.point_count} points, '
        f'sha256={asset.pcd_sha256}'
    )
    return 0
