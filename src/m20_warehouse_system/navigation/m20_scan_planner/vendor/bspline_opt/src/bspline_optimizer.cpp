...
namespace scan_planner
{
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
      ...
      {
        RCLCPP_WARN(rclcpp::get_logger("bspline_opt"),
                    "Solver error. Return=%d, %s; using the collision-free seed trajectory",
                    result, lbfgs::lbfgs_strerror(result));
        // The A* seed has already passed the occupancy checks above.  A
        // line-search failure is a numerical optimization issue, not a
        // collision verdict; keep the valid seed so one bad solve does not
        // discard the complete navigation command.
