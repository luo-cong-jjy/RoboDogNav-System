...
## Factory real-robot transports

The guarded hardware boundary supports exactly one of two factory backends:

- `basic_server` (default) uses the documented APDU/JSON TCP/UDP protocol and
  vendor-managed state progression;
- `direct_ros` publishes `/MOTION_STATE`, `/GAIT`, and `/NAV_CMD`, subscribes
  to the 20 Hz `/MOTION_INFO`, and explicitly confirms state 17 and gait
  `0x3002` before reporting ready.

Both consume `/m20/locomotion/cmd_vel_sdk`, publish the same ready/fault and
`/m20/locomotion/measured_twist` topics, enforce a 300 ms local command
timeout, and start with motion disabled. Select a backend only on the Foxy
hardware launch; MuJoCo continues to use the local ONNX joint controller.

Both factory backends fail closed on the M20 physical hard-e-stop status.
`basic_server` reads `BasicStatus.HES`; `direct_ros` subscribes to
`/HES_STATUS` (`drdds/msg/StdMsgInt32.value`). Releasing the tail switch never
reenables motion automatically: after inspection an operator must call
`/m20/hardware/enable_motion` with `false`, then explicitly request `true`.
The local `/m20/control/e_stop` remains a separate software soft-e-stop and
does not replace the physical switch.

Build and test the isolated adapter:

```bash
colcon build --symlink-install --packages-select m20_nav2_locomotion
colcon test --packages-select m20_nav2_locomotion
```

Run only the safe command adapter:

```bash
ros2 launch m20_nav2_locomotion sdk_locomotion.launch.py
```
...
```bash
ros2 launch m20_nav2_locomotion sdk_locomotion.launch.py start_sdk:=true
```
