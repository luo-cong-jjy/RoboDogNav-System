# ============================================================================
# 文件：pcd_to_nav2_costmap.launch.py
# 功能：PCD → Nav2 代价地图 launch：把离线 PCD 点云数据接入 Nav2 costmap。
#       流程：
#         - 发布静态 TF（odom→base_link→lidar_link，占位单位变换）
#         - 包含点云转扫描 launch（PCD 切片点云 → /scan）
#         - 启动 Nav2 costmap（nav2_costmap_2d，加载 nav2_costmap_from_bag.yaml）
#         - 2 秒后激活 costmap 生命周期节点
#         - 7 秒后启动 pcd_slice_publisher 逐帧回放 PCD 切片点云
#         - 可选启动 RViz（lidar_to_scan 配置）
#       与 pcd_validation.launch.py 内容一致，主要用于验证
#       「离线点云 → 2D 扫描 → 代价地图」链路。
# ============================================================================
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # —— 启动 Nav2 costmap 节点 ——
        Node(  # 启动 nav2_costmap_2d，用 /scan 数据构建代价地图
        ...
        # —— 可选启动 RViz ——
        Node(  # 启动 RViz 查看点云/扫描/costmap 结果
            condition=IfCondition(use_rviz),  # 仅当 use_rviz=true 时才启动
            package="rviz2",  # 所属功能包
            executable="rviz2",  # 可执行文件名
            name="rviz2_nav2_costmap_from_pcd",  # 节点名（标识为 PCD→costmap 场景）
            output="screen",  # 日志输出到屏幕
