...
```bash
src/m20_nav2_system/maps/pcd/raw/global.pcd
src/m20_nav2_system/maps/pcd/raw/0.pcd
src/m20_nav2_system/maps/pcd/raw/t100ipro_2026-07-14-11-56-57.pcd
src/m20_nav2_system/maps/factory/factory_slam_map.yaml
src/m20_nav2_system/maps/factory/factory_slam_map.pgm
```
...
```bash
ros2 launch m20_nav2_system bag_to_scan_test.launch.py
```
...
ros2 launch m20_nav2_system bag_to_scan_rviz.launch.py
```
...
```bash
export ROS_LOCALHOST_ONLY=1
ros2 launch m20_nav2_system bag_to_nav2_costmap.launch.py
```
...
```bash
export ROS_LOCALHOST_ONLY=1
ros2 launch m20_nav2_system pcd_to_nav2_costmap.launch.py use_rviz:=false
```
...
```bash
ros2 launch m20_nav2_system pcd_to_nav2_costmap.launch.py use_rviz:=true
```
...
python3 src/m20_nav2_system/scripts/pcd/pcd_to_occupancy_grid \
  --pcd src/m20_nav2_system/maps/pcd/raw/t100ipro_2026-07-14-11-56-57.pcd \
  --output src/m20_nav2_system/maps/pcd/grids/t100ipro_grid.yaml \
  --resolution 0.20 \
  --min-height -0.30 \
  --max-height 0.30 \
  --inflate-radius 0.30 \
...
```bash
python3 src/m20_nav2_system/scripts/pcd/grid_astar_planner \
  --map src/m20_nav2_system/maps/pcd/grids/t100ipro_grid.yaml \
  --output-path src/m20_nav2_system/maps/pcd/grids/t100ipro_grid_astar_path.csv \
  --overlay src/m20_nav2_system/maps/pcd/grids/t100ipro_grid_astar_overlay.ppm
```
...
source /opt/ros/humble/setup.bash
source install/setup.bash
ros2 run m20_nav2_system compute_path_probe
```
...
```bash
ros2 launch m20_nav2_system pointcloud_to_scan.launch.py
```

如果已经发布雷达到 `base_link` 的 TF，可以加：

```bash
ros2 launch m20_nav2_system pointcloud_to_scan.launch.py target_frame:=base_link
```
