# ============================================================================
# 文件：factory_inspection_mission_mujoco.launch.py
# 功能：启动 MuJoCo 专用 M20 工厂巡检任务节点。
#       该节点读取 MuJoCo 巡检途经点参数文件，
#       在 Nav2 导航栈就绪后，依次向 /m20_nav2/goal_pose_raw 发布目标点，
#       驱动 M20 机器人按途经点顺序完成工厂巡检。
#       注意：本 launch 只启动任务节点，不含仿真/Nav2，
#       需在 MuJoCo 仿真与 Nav2 导航栈启动之后单独运行。
# ============================================================================
...
def generate_launch_description():
    ...
    default_params = str(pkg_share / "config" / "factory_inspection_midpoints_mujoco.yaml")
    ...
    return LaunchDescription([
        ...
        # MuJoCo backend publishes wall-clock timestamps and does not publish
        # /clock, so this entry point uses wall time.
        DeclareLaunchArgument("use_sim_time", default_value="false"),

        Node(  # 启动工厂巡检任务节点
            package="m20_nav2_system",  # 所属功能包
            executable="factory_inspection_nav2_mission_mujoco",
            name="factory_inspection_nav2_mission_mujoco",
            output="screen",  # 日志输出到屏幕
            parameters=[params_file, {"use_sim_time": use_sim_time}],  # 加载途经点、等待时间等任务参数
        ),
    ])
