...
def _make_gazebo_spawn_urdf(pkg_share: Path, source_urdf: Path) -> Path:
    ...
    Gazebo Classic's URDF importer may rewrite ``package://`` mesh URIs to
    ``model://`` even when ``spawn_entity`` was given a package URI.  The
    resulting lookup requires a model.config and silently drops the visual
    meshes.  Absolute paths avoid that conversion while keeping the source
    URDF portable for robot_state_publisher and RViz.
    """
...
def generate_launch_description():
    ...
    gazebo_robot_source_urdf = pkg_share / "models" / "m20_gazebo_2d_scan.urdf"
    ...
    pointcloud_to_scan_launch = pkg_share / "launch" / "rslidar_pointcloud_to_scan_gazebo.launch.py"  # Gazebo 点云转激光扫描入口（本模式下默认不使用）
    ...
    use_rviz = LaunchConfiguration("gazebo_use_rviz")  # 是否由 Gazebo 传感器子 launch 启动 RViz
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("gazebo_use_rviz", default_value="false"),  # 默认不启动 RViz（由上层 launch 统一管理）
        ...
        # Gazebo-only dynamic avoidance: infer moving scan clusters from the
        # measured LaserScan and static map. No world trajectory is read.
        Node(
        ...
        # —— 启动 Gazebo 服务器 ——
        LogInfo(msg=['[m20_nav2_system] gzserver world: ', world]),
