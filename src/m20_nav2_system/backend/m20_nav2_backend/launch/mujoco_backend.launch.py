...
#         围墙），输出到 /tmp 缓存路径；
#       - 启动 m20_nav2_backend 物理节点（无头，use_viewer=False，
...
from m20_nav2_backend.world_generator import (
    cached_world_path,
    generate_world,
)
...
def _runtime_actions(context):
    ...
    # 包共享目录：本包（模型模板/配置）与 m20_nav2_description（网格）。
    backend_share = Path(
        get_package_share_directory('m20_nav2_backend')
    )
    ...
    return [
        # ---------- 1) 物理后端节点（无头） ----------
        Node(
            package='m20_nav2_backend',
            executable='m20_nav2_backend',
            name='m20_nav2_backend',
            output='screen',
            parameters=[
